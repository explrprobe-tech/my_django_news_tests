...
def test_categories_unauthenticated_user_get(unauthenticated_session, base_url, test_category_url, test_category_data):
    ...
    assert test_category_data["title"] in response_categories_list.text, "Created category is not on category page"
    response_category = unauthenticated_session.get(url=test_category_url)
    assert response_category.status_code == 200, "Unauthenticated user should have access to category"
    assert test_category_data["title"] in response_categories_list.text, "Created category is not on category page"
def test_categories_regular_user_get(regular_session, base_url, test_category_url, test_category_data):
    ...
    assert test_category_data["title"] in response_categories_list.text, "Created category is not on category page"
    response_category = regular_session.get(url=test_category_url)
    assert response_category.status_code == 200, "Regular user should have access to category"
    assert test_category_data["title"] in response_category.text, "Created category is not on category page"
def test_categories_editor_user_get(editor_session, base_url, test_category_url, test_category_data):
    ...
    assert test_category_data["title"] in response_categories_list.text, "Created category is not on category page"
    response_category = editor_session.get(url=test_category_url)
    assert response_category.status_code == 200, "Editor user should have access to category"
    assert test_category_data["title"] in response_categories_list.text, "Created category is not on category page"
def test_categories_admin_user_get(admin_session, base_url, test_category_url, test_category_data):
    ...
    assert test_category_data["title"] in response_categories_list.text, "Created category is not on category page"
    response_category = admin_session.get(url=test_category_url)
    assert response_category.status_code == 200, "Admin user should have access to category"
    assert test_category_data["title"] in response_categories_list.text, "Created category is not on category page"
def test_categories_unauthenticated_user_post(unauthenticated_session, base_url, test_category_data):
    """Unauthenticated user can't create category"""
    url = f"{base_url}/category/add_category/"
    response = unauthenticated_session.post(url=url, data=test_category_data)
    assert response.status_code == 403, "Unauthenticated user shouldn't have access to create category"
def test_categories_regular_user_post(regular_session, base_url, test_category_data):
    """Unauthenticated user can't create category"""
    url = f"{base_url}/category/add_category/"
    response = regular_session.post(url=url, data=test_category_data)
    assert response.status_code == 403, "Regular user shouldn't have access to create category"
def test_categories_editor_user_post(editor_session, base_url, test_category_data):
...
def test_categories_admin_user_post(admin_session, base_url, test_category_data):
    """Unauthenticated user can't create category"""
    url_create = f"{base_url}/category/add_category/"
