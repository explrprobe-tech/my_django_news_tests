...
def test_create_news(request, news_api, news_data, session_name, expected_status, msg_err):
    ...
    create_news_response.status_code == expected_status, msg_err

@pytest.mark.parametrize("session_name, expected_status, msg_err", [
    ("unauthenticated_session", 403, "Unathenticated user shouldn't have access to delete news"),
    ("regular_session", 403, "Regular user shouldn't have access to delete news"),
    ("editor_session", 200, "Editor user should have access to delete news"),
    ("admin_session", 200, "Admin user should have access to delete news")
])
def test_delete_news(request, news_api, test_news, session_name, expected_status, msg_err):
    """Only admin and editor users can delete news"""
    session = request.getfixturevalue(session_name)
    news_api.set_session(session)
    delete_news_response = news_api.delete_news(test_news["test_news_id"])
